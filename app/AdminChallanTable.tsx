...
import { useState, useMemo } from "react"; // useMemo for calculating stats
...
export function AdminChallanTable({ challans }: AdminChallanTableProps) {
  ...
  // Sort challans by date, most recent first
  ...
  {/* Challan Table */ }
  {
    sortedChallans.length === 0 ? (
      ...
      <div className="overflow-x-auto" >
        ...
        <TableHeader className="bg-gray-900" >
          <TableRow>
          <TableHead className="text-gray-300" > ID < /TableHead> {/ * Added ID * /}
            < TableHead className = "text-gray-300" > Date < /TableHead>
              < TableHead className = "text-gray-300" > Plate No.< /TableHead> {/ * Added Plate No. * /}
                < TableHead className = "text-gray-300" > Name < /TableHead> {/ * Added Name * /}
                          ...
                          <TableBody>
    {
      sortedChallans.map((challan) => (
        // Use challan.id as key if available, fallback to index
        <TableRow key= { challan.id || challans.indexOf(challan) } className = "border-t border-gray-700" >
        <TableCell className="font-medium text-xs" > { challan.id?.substring(0, 6) || 'N/A' } < /TableCell> {/ * Display truncated ID * /}
        < TableCell > { formatDate(challan.date)
    } </TableCell>
      < TableCell > { challan.plateNumber } < /TableCell> {/ * Display Plate No. * /}
        < TableCell > { challan.name } < /TableCell> {/ * Display Name * /}
          < TableCell > { challan.violation } < /TableCell>
          < TableCell className = "text-right" >₹{ challan.fineAmount } </TableCell>
            < TableCell className = "text-right" >
              <span className={
                `px-2 py-1 rounded-full text-xs ${challan.status === "Paid"
...
{
  selectedChallan && (
    <div className="mt-6" > {/* Added a container for spacing */ }
      < UserDetailsModal details = { selectedChallan } />
        <div className="flex justify-end mt-4" >
          {/* Add a close button for the modal display */ }
          < Button
  variant = "outline"
